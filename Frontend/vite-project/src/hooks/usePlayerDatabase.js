...
function usePlayerDatabase() {
  ...
  const updatePlayerStats = useCallback((jersey, stats) => {
    ...
    if (!pendingStatsRef.current[key]) {
      pendingStatsRef.current[key] = {
        ...
        dotBalls: 0, dotBallsBowled: 0, ducks: 0,
        ones: 0, twos: 0, threes: 0, thirties: 0, fifties: 0, hundreds: 0,
        wides: 0, noBalls: 0, maidens: 0,
        threeWickets: 0, fiveWickets: 0, tenWickets: 0,
        highestScore: 0, dismissals: 0, notOuts: 0,
        matches: 0,
        captainMatches: 0, captainWins: 0, captainLosses: 0,
        captainTies: 0, captainNR: 0,
      };
    }
    ...
    const addFields = [
      ...
      "ones","twos","threes","thirties","fifties","hundreds",
      "ducks","dismissals","notOuts","wides","noBalls","maidens",
  ...
  const updateMatchMilestones = useCallback(async () => {
    // Flush fielding stats into pending buffer
    for (const [name, fielding] of Object.entries(fieldingBufferRef.current)) {
      ...
      try {
        ...
        _cache[player.jersey || player._id] = { ..._cache[player.jersey || player._id], ...fielding };
      } catch (err) {
    ...
    for (const [key, buf] of Object.entries(pendingStatsRef.current)) {
      ...
      try {
        ...
        // Refresh from server so virtuals (averages) are up to date
  ...
  return {
    ...
    // Team stats (still localStorage until Day 10)
    updateTeamStats,
  };
}
