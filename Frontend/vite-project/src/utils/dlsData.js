// DLS Resource Percentage Table (Standard Edition)
// Based on ICC Duckworth-Lewis-Stern Method
// Table shows resource percentages remaining for each combination of overs left and wickets lost
...
// G50 values based on ICC guidelines
const G50_VALUES = {
  international: 245,      // ODI, Test nations
  lower: 200,             // U19, U15, Women's, Associate nations
};

/**
 * Get resource percentage from DLS table
 * @param {number} oversLeft - Overs remaining
 ...
 * @returns {Object} Calculation result with revised target and explanation
 */
function calculateDLSTarget(params) {
  const {
    team1Score,           // Team 1's total score
    team1OversAllocated,  // Overs allocated to Team 1 at start
    team1OversUsed,       // Overs used by Team 1
    team1WicketsLost,     // Wickets lost by Team 1
    team2OversAllocated,  // Overs allocated to Team 2 at start
    team2OversUsed,       // Overs currently used by Team 2
    team2WicketsLost,     // Current wickets lost by Team 2
    g50 = G50_VALUES.international  // G50 value (245 for international, 200 for lower levels)
  } = params;

  // Calculate Team 1's resource percentage
  const team1OversLeft = team1OversAllocated - team1OversUsed;
  const team1ResourceStart = getResourcePercentage(team1OversAllocated, 0);
  const team1ResourceEnd = getResourcePercentage(team1OversLeft, team1WicketsLost);
  const team1ResourceUsed = team1ResourceStart - team1ResourceEnd;
  const R1 = team1ResourceUsed;

  // Calculate Team 2's resource percentage
  const team2OversLeft = team2OversAllocated - team2OversUsed;
  const team2ResourceAvailable = getResourcePercentage(team2OversLeft, team2WicketsLost);
  const R2 = team2ResourceAvailable;
  ...
  // Case 2: R2 = R1 (Equal resources)
  else if (Math.abs(R2 - R1) < 0.1) {
    revisedTarget = team1Score + 1;
    explanation = `Both teams have equal resources (${R1.toFixed(1)}%). Target is Team 1's score + 1.`;
  }
  // Case 3: R2 > R1 (Team 2 has more resources)
  else {
    const excessResource = R2 - R1;
    const extraRuns = Math.floor((excessResource * g50) / 100);
    revisedTarget = team1Score + extraRuns + 1;
...
if (team2OversUsed > 0) {
  ...
  return {
    ...
    details: {
      team1: {
        resourceStart: team1ResourceStart,
        resourceEnd: team1ResourceEnd,
        resourceUsed: R1
      },
      team2: {
        resourceAvailable: R2,
        oversLeft: team2OversLeft
      }
    }
  };
}

export { DLS_TABLE, G50_VALUES, getResourcePercentage, calculateDLSTarget };
