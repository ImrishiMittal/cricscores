...
function MatchSummary({
  ...
  const determineResult = () => {
    ...
    // ✅ Both scores equal AND no super over = tie
    if (score1 === score2 && !isSuperOver) {
      return {
        type: "tie",
    ...
    // ✅ Super over winner suffix
    const superOverSuffix = isSuperOver
      ? superOverNumber > 1
    ...
    if (score1 > score2) {
      return {
        type: "win",
        message: `${team1} WON BY ${score1 - score2} RUNS${superOverSuffix}`,
    ...
    return {
      type: "win",
      message: `${team2} WON BY ${wicketsRemaining} WICKET${
  ...
  const team1OversFormatted = formatOvers(
    innings1Score?.overs || 0,
    innings1Score?.balls || 0
  );
  const team2Score = innings2Score?.score || 0;
  const team2Wickets = innings2Score?.wickets || 0;
  const team2OversFormatted = formatOvers(
    innings2Score?.overs || 0,
    innings2Score?.balls || 0
  );
      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.momContent}>
              ...
              <div className={styles.momStats}>
                {manOfTheMatch.battingScore > 0 && (
                  <span className={styles.momStat}>
                    Batting: {manOfTheMatch.battingScore} pts
                  </span>
                )}
                {manOfTheMatch.bowlingScore > 0 && (
                  <span className={styles.momStat}>
                    Bowling: {manOfTheMatch.bowlingScore} pts
                  </span>
                )}
                <span className={styles.momTotal}>
                  Total: {manOfTheMatch.totalScore} pts
                </span>
              </div>
            </div>
          </div>
        ...
        <div className={styles.scorecardContainer}>
          ...
          <div className={styles.scorecard}>
            ...
            {innings1Data?.battingStats &&
              innings1Data.battingStats.length > 0 && (
            ...
            {innings1Data?.bowlingStats &&
              innings1Data.bowlingStats.length > 0 && (
                <div className={styles.statsTable}>
                  <div className={styles.tableHeader}>
          ...
          <div className={styles.scorecard}>
            ...
            {innings2Data?.battingStats &&
              innings2Data.battingStats.length > 0 && (
                <div className={styles.statsTable}>
                  <div className={styles.tableHeader}>
                    <div className={styles.playerCol}>BATSMAN</div>
                    <div className={styles.statCol}>R</div>
            ...
            {innings2Data?.bowlingStats &&
              innings2Data.bowlingStats.length > 0 && (
                <div className={styles.statsTable}>
                  <div className={styles.tableHeader}>
                    <div className={styles.playerCol}>BOWLER</div>
                    <div className={styles.statCol}>O</div>
