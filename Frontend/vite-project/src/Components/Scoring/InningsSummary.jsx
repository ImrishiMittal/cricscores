        ...
        <div className={styles.section}>
          ...
          <div className={styles.table}>
            ...
            {bowlers.map((bowler, idx) => {
              ...
              const totalOversNum =
                Math.floor(ballsBowled / 6) + (ballsBowled % 6) / 6;
              const economy =
                totalOversNum > 0
                  ? (
                      (bowler.runsGiven ?? bowler.runs ?? 0) / totalOversNum
                    ).toFixed(2)
                  : "0.00";
              return (
                <div key={bowler.playerId || idx} className={styles.tableRow}>
                  <div className={styles.playerCol}>{bowler.displayName}</div>

                  <div className={styles.statCol}>
                    {Math.floor((bowler.ballsBowled || 0) / 6)}.
                    {(bowler.ballsBowled || 0) % 6}
                  </div>

                  <div className={styles.statCol}>
                    {bowler.runsGiven ?? bowler.runs ?? 0}
                  </div>
